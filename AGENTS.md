...
- Aha: Remote registry advertisement is an optional daemon-side control-plane client: local services are upserted by namespace/alias at startup and refreshed by heartbeat, while remote outages must remain non-fatal to local supervision.
...
- Aha: The repository has no Wrangler/Worker entrypoint or Cloudflare resource identifiers; the central registry remains an in-process Go daemon, so Cloudflare deployment requires external compute behind a named Tunnel rather than a Worker-only workflow.
